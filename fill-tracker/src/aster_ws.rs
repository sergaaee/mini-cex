...
async fn keepalive_listen_key(api_key: &str) {
    let client = reqwest::Client::new();
    let _ = client
        .put("https://fapi.asterdex.com/fapi/v1/listenKey")
        .header("X-MBX-APIKEY", api_key)
        .send()
        .await;
}
...
pub async fn run(tracker: Arc<Mutex<FillTracker>>, api_key: String) {
    loop {
        match run_once(&tracker, &api_key).await {
            Ok(_) => warn!("Aster WS stream ended, reconnecting..."),
            Err(e) => error!("Aster WS error: {}, reconnecting...", e),
        }
...
async fn run_once(tracker: &Arc<Mutex<FillTracker>>, api_key: &str) -> anyhow::Result<()> {
    ...
    info!(listen_key = %&listen_key[..8], "Got Aster listenKey, connecting WS");

    let url = format!("wss://fstream.asterdex.com/private/ws/{}", listen_key);
    let request = url.clone().into_client_request()?;
    let (mut ws_stream, _) = connect_async(request).await?;
    info!("Connected to Aster user stream");
